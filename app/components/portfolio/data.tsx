...
export const NAV = [
  ...
  { href: "#why", label: "Why Hire Me", icon: <ShieldCheck className="h-4 w-4" /> },
  { href: "#achievements", label: "Achievements", icon: <BarChart3 className="h-4 w-4" /> },
  { href: "#experience", label: "Experience", icon: <Briefcase className="h-4 w-4" /> },
  { href: "#skills", label: "Skills", icon: <Wrench className="h-4 w-4" /> },
  { href: "#projects", label: "Projects", icon: <FolderGit2 className="h-4 w-4" /> },
  { href: "#education", label: "Education", icon: <GraduationCap className="h-4 w-4" /> },
  { href: "#certifications", label: "Certifications", icon: <FileText className="h-4 w-4" /> },
...
export const certGroups: { title: string; items: string[] }[] = [
  { title: "Advanced (ML / DL)", items: ["Machine Learning and Deep Learning in Python & R", "Intermediate Machine Learning"] },
  {
    title: "Foundations (Core ML + Python)",
    items: ["Machine Learning A–Z: Python & R", "Python for Data Science and Machine Learning Bootcamp", "Introduction to Machine Learning", "Python Programming"],
  },
  { title: "Applied (Data / Analytics)", items: ["Pandas for Data Analysis", "PostgreSQL Essentials", "Data Visualization", "CV Masterclass"] },
];

export const achievements: { title: string; note: string; icon: ReactNode }[] = [
  { title: "~15% forecasting improvement", note: "Built + evaluated predictive models in Python/SQL", icon: <Target className="h-5 w-5 text-black/70" /> },
  { title: "10%+ cost reduction impact", note: "Operational insights → measurable savings", icon: <BarChart3 className="h-5 w-5 text-black/70" /> },
  ...
  { title: "Production ML systems", note: "FastAPI + Streamlit + Docker + CI/CD", icon: <ShieldCheck className="h-5 w-5 text-black/70" /> },
];
...
export const projects: Project[] = [
  {
    ...
    tags: ["Next.js", "FastAPI", "Customer Segmentation", "Clustering", "Run Management", "Simulation Engine", "REST APIs", "Swagger", "Vercel", "Render"],
    links: {
...

  {
    ...
    tags: ["Python", "Streamlit", "PyTorch", "Grad-CAM", "OpenCV/Pillow", "Risk Stratification", "ReportLab", "Ruff", "GitHub Actions", "Clinical Workflow"],
    links: { code: "https://github.com/Gujjar-Pranav/retina-ai", demo: "https://retina-ai-zpkddbsb6m2rf6tfgd6rjh.streamlit.app" },
...

  {
    ...
    screenshots: ["/projects/glass_indetification_1.png", "/projects/glass_indetification_2.png", "/projects/glass_indetification_3.png", "/projects/glass_indetification_4.png"],
    highlights: ["Advanced feature engineering + imbalance handling", "Stacking ensemble selected over tuned baselines", "FastAPI inference service with trained artifacts", "Dockerized services with CI builds"],
    architecture: ["UCI data → cleaning → outlier handling → features", "Scaling + stratified split + SMOTE → training/eval", "Persist model + scaler + schema artifacts", "FastAPI /predict endpoint → Streamlit UI calls API"],
  },

  {
    title: "Meeting Task Assignment",
    icon: <Mic className="h-5 w-5 text-black/70" />,
    description: "Local-only pipeline that turns meeting audio into structured task JSON using offline Whisper speech-to-text + rule-based NLP (no cloud/APIs).",
    badges: [
...
export const skillGroups: SkillGroup[] = [
  { title: "Core", subtitle: "Primary tools used across projects", items: ["Python", "SQL", "scikit-learn", "PyTorch", "TensorFlow", "FastAPI", "Streamlit", "Docker"] },
  {
    title: "Machine Learning",
    subtitle: "Modeling + evaluation in real workflows",
    items: ["Supervised/Unsupervised Learning", "Feature Engineering", "Model Evaluation (ROC-AUC, Precision/Recall)", "Calibration & Confidence Scoring", "Hyperparameter Tuning"],
  },
  { title: "NLP & Speech", subtitle: "Text pipelines + speech-to-text", items: ["TF-IDF", "Linear SVM", "Text Preprocessing", "Rule-based NLP", "Speech-to-Text (Whisper)"] },
  { title: "Computer Vision", subtitle: "Segmentation + image workflows", items: ["U-Net", "Image Segmentation", "Augmentation", "Dice/IoU", "Preprocessing (CLAHE)"] },
